...
@enduml"""
    ...
    def seq(self):
        ...
        def _sanitize_plantuml(s):
            """Sanitize string for PlantUML output to prevent command injection."""
            if not s:
                return s
            # Remove newlines and carriage returns (prevent new command injection)
            s = s.replace('\n', ' ').replace('\r', ' ')
            # Remove double quotes (prevent breaking out of string context)
            s = s.replace('"', "'")
            return s
        ...
        for e in TM._elements:
            if isinstance(e, Actor):
                participants.append(
                    'actor {0} as "{1}"'.format(
                        e._uniq_name(), _sanitize_plantuml(getattr(e, "display_name", lambda: e.name)())
                    )
                )
        ...
        for e in TM._flows:
            message = "{0} -> {1}: {2}".format(
                e.source._uniq_name(),
                e.sink._uniq_name(),
                _sanitize_plantuml(getattr(e, "display_name", lambda: e.name)()),
            )
            note = ""
            if getattr(e, "note", "") != "":
                note = "\nnote left\n{}\nend note".format(_sanitize_plantuml(e.note))
    ...
    def report(self, template_path):
        ...
        # Create HTML-escaped clone of TM object
        tm_copy = _clone(self)
        for attr_name in self.__class__.model_fields:
            value = getattr(self, attr_name, None)
            if isinstance(value, str):
                object.__setattr__(tm_copy, attr_name, html_escape(value))
        ...
        data = {
            ...
            "data": encode_element_threat_data(TM._data),
        }
