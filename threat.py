...
class _ConditionValidator(ast.NodeVisitor):
    ...
    def visit_BinOp(self, node: ast.BinOp) -> Any:  # noqa: D401
        # Disallow sequence repetition (list/tuple/string * int) as it can create
        # arbitrarily large data structures causing memory exhaustion
        if isinstance(node.op, ast.Mult):
            for operand in (node.left, node.right):
                if isinstance(operand, (ast.List, ast.Tuple)):
                    raise ValueError(
                        "Sequence repetition with * is not permitted in threat conditions"
                    )
                if isinstance(operand, ast.Constant) and isinstance(operand.value, str):
                    raise ValueError(
                        "String repetition with * is not permitted in threat conditions"
                    )
        return self.generic_visit(node)

    def visit_ListComp(self, node: ast.ListComp) -> Any:  # noqa: D401
        # Disallow nested comprehensions (multiple for clauses) as they can cause
        # exponential memory/time consumption
        if len(node.generators) > 1:
            raise ValueError(
                "Nested comprehensions are not permitted in threat conditions"
            )
        return self.generic_visit(node)

    def visit_GeneratorExp(self, node: ast.GeneratorExp) -> Any:  # noqa: D401
        # Disallow nested generators (multiple for clauses) as they can cause
        # exponential time consumption
        if len(node.generators) > 1:
            raise ValueError(
                "Nested generators are not permitted in threat conditions"
            )
        return self.generic_visit(node)
