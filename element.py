...
def _escape_dot_string(s: str) -> str:
    """Escape a string for use in DOT double-quoted strings.

    Escapes backslashes and double quotes to prevent DOT injection.
    """
...
def _escape_html(s: str) -> str:
    """Escape a string for use in DOT HTML-like labels.

    Escapes &, <, and > to prevent HTML injection in GraphViz labels.
    """
...
"""
    ...
    def _label(self) -> str:
        ...
        Returns an escaped string safe for use in DOT double-quoted strings.
        The display name is escaped before being wrapped and joined with DOT newlines.
        """
        # Escape user-controlled content first, then wrap and join with DOT newlines
        escaped_name = _escape_dot_string(self.display_name())
        return "\\n".join(wrap(escaped_name, 18))
